...
class Admin extends User {
    ...
    public static void viewAttendee(){
        Database.getAttendees().forEach(attendee -> System.out.print(attendee.getUsername() + " - " + attendee.getBalance() + "\n"));
    }
...
while (addr == null || addr.length() < 1) {
    ...
    public String bookEvent(String input) {
        ...
        if (balance >= chosenEvent.getTicketPrice()) {
            balance -= chosenEvent.getTicketPrice();
            outputArea2.append("Booked: " + chosenEvent.getName() + "\n");
            outputArea2.append("New balance: $" + balance + "\n");
        } else {
...
Database db = Database.getInstance();
    ...
    class Wallet {
        ...
        Wallet(String waletId ,double balance,String userId){
            this.walletId = waletId;
            this.balance = balance;
            this.userId = userId;
        }
    }
